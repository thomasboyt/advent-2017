...
const registers = {
  ...
  add(name, amount) {
    const nextAmount = registers.get(name) + amount;

    if (nextAmount > (_maxRegisters.get(name) || 0)) {
      _maxRegisters.set(name, nextAmount);
    }

    _registers.set(name, nextAmount);
  },

  max() {
    return Math.max(..._maxRegisters.values());
  }
};
